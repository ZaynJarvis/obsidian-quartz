---
course: AI Builders：从用户到构建者
chapter: 实战项目：五个真实项目
source: https://www.superlinear.academy/c/ai/sections/334458/lessons/1240535
scraped_at: 2026-06-28
video: true
---

# Office Hours：课程项目实战讲解

## 我的理解

这是 Maven Office Hours 的录播视频，可供参考的文字内容极少。视频开头演示了 Codex 的使用方式，随后简要介绍了 AI Architect 课程附带的 Builder Space 功能；未购买该课程的学员可直接跳过 Builder Space 部分。

## 相关链接

- [[Ch03-L01 概述]] — 本次 Office Hours 围绕第三模块的实战项目展开答疑与演示
- [[Ch08-L02 Cursor 入门 你的 AI 编程搭档]] — Codex 与 Cursor 同属 AI 辅助编程工具，使用思路相近

---

## 原文

Lesson 24 of 68
视频 - Office Hours：课程项目实战讲解 / Video - Office Hours: Walkthrough of class projects

最后更新：2026 年 5 月 7 日

这是我们 Maven Office Hour 的录播。

在最开始，我们演示了如何使用 Codex。之后，我们简单介绍了 Builder Space。Builder Space 是我们随 AI Architect 课程一起提供的内容。如果你没有购买 AI Architect 课程，或者对 Builder Space 不感兴趣，可以直接跳过这一部分。

English Original

Last updated: May 7th, 2026

This is the recording from our Maven Office Hour.

At the very beginning, we walk through how to use Builder Space. Builder Space is something we offer as part of the AI Architect course. If you didn’t buy the AI Architect course or you’re not interested in Builder Space, feel free to skip this part.